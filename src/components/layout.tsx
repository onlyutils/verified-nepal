      ...
      <header className={shell}>
        <div className="flex items-center justify-between gap-4 py-3">
          <button type="button" onClick={() => navigate("dashboard")} className={`flex items-center gap-2 ${focusRing}`}>
            ...
            <span className="hidden font-sans text-[0.68rem] uppercase tracking-[0.14em] text-muted-foreground sm:inline">{ts.compactBadge}</span>
          </button>
  ...
  return (
    <header className={shell}>
      ...
      <div className="flex items-center justify-between gap-2 py-2.5 lg:hidden">
        <div className="flex min-w-0 flex-1 flex-col items-start overflow-hidden font-sans text-[0.6rem] uppercase tracking-[0.1em] text-muted-foreground">
      ...
      <div className="hidden items-center gap-4 py-5 text-left lg:grid lg:grid-cols-[1fr_auto_1fr]">
        <div className="flex min-w-0 flex-col items-start font-sans text-[0.68rem] uppercase tracking-[0.14em] text-muted-foreground">
...
function EditionLine({ language, compact = false }: { language: Language; compact?: boolean }) {
  ...
  return (
    <p className="flex flex-wrap items-center justify-end gap-x-1.5 font-sans text-[0.68rem] uppercase tracking-[0.14em] text-muted-foreground">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-0 border-b border-rule font-sans text-[0.72rem] uppercase tracking-[0.14em]">
        ...
        {numbers.map(([number, label]) => (
          <a key={number} href={`tel:${number}`} className={`inline-flex min-h-11 items-center gap-1.5 text-ink ${focusRing}`}>
            ...
            <span className="normal-case tracking-normal text-muted-foreground">{label}</span>
          </a>
        ))}
      </div>
    </aside>
  ...
  return (
    <footer className={`${shell} pb-24`}>
      ...
      <div className="grid gap-8 py-8 font-serif text-sm leading-6 lg:grid-cols-[1.4fr_1fr_1fr]">
        ...
        <div className="font-sans text-[0.72rem] uppercase leading-6 tracking-[0.14em] text-muted-foreground">
      ...
      <Rule />
      <div className="flex flex-col gap-2 pt-4 font-sans text-[0.68rem] uppercase tracking-[0.14em] text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        <p>
...
export function AccessibilityBar({
  ...
  const control = `inline-flex min-h-11 min-w-11 items-center justify-center px-2 font-sans text-[0.72rem] font-semibold tracking-[0.08em] text-ink transition-colors hover:bg-ink hover:text-paper disabled:cursor-not-allowed disabled:text-muted-foreground disabled:hover:bg-transparent disabled:hover:text-muted-foreground ${focusRing}`;

  const controls = (
    <>
      <span className="mr-2 font-sans text-[0.62rem] uppercase tracking-[0.18em] text-muted-foreground">{t.accessibility}</span>
