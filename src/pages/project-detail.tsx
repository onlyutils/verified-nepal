...
export function ProjectDetail({ language, id }: { language: Language; id: string }) {
  ...
  if (loading) return <p className="font-sans text-sm text-muted-foreground">{t.projectDetailLoading}</p>;
  if (error) return <div className="border border-rule bg-card px-4 py-6" role="alert"><p className="font-sans text-sm text-destructive">{error}</p>{offline ? <p className="mt-1 font-sans text-xs text-muted-foreground">{t.projectsOffline}</p> : null}<div className="mt-4"><a href="/projects" className="font-sans text-sm underline">{t.projectDetailBack}</a></div></div>;
  if (!project) return <p className="font-sans text-sm text-muted-foreground">{t.projectDetailNotFound}</p>;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <header className="border-b border-rule pb-6">
        ...
        <p className="mt-2 font-sans text-sm text-muted-foreground">{districtLabels[project.district as keyof typeof districtLabels]?.[language] ?? project.district} · W{project.ward} · {project.locationText} · {fillTemplate(t.projectsCostNpr,{amount: formatNumber(project.costEstimateNpr, language)})}</p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.7fr_1fr]">
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              {project.photos.length===0 ? <p className="font-sans text-sm text-muted-foreground">{t.projectDetailNoPhotos}</p> :
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between"><CardTitle className="text-base">{t.projectDetailUpdatesTitle}</CardTitle><span className="font-sans text-xs text-muted-foreground">{project.updates.length}</span></CardHeader>
            <CardContent className="space-y-4">
              {project.updates.length===0 ? <p className="font-sans text-sm text-muted-foreground">{t.projectDetailNoUpdates}</p> :
                project.updates.slice().sort((a,b)=> new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(u=>(
                  <div key={u.id} className="border border-rule bg-card p-4">
                    <p className="font-sans text-xs text-muted-foreground">{formatDateTime(u.createdAt, language)} {u.spentNpr!=null ? `· ${fillTemplate(t.projectDetailSpentNpr,{amount: formatNumber(u.spentNpr, language)})}` : ""}</p>
        ...
        <div className="space-y-6">
          <Card className="border-ink">
            <CardHeader>
              ...
              {project.committee.verified ? <p className="font-sans text-xs font-semibold uppercase tracking-wide text-blue">{t.projectDetailVerifiedBadge}</p> : <p className="font-sans text-xs uppercase tracking-wide text-muted-foreground">{t.projectDetailNotVerified}</p>}
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="font-sans text-xs uppercase tracking-wide text-muted-foreground">{t.projectDetailCommittee}</p>
                ...
                <>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 font-sans text-sm">
                      <div className="flex items-center justify-between gap-2"><span className="text-muted-foreground">{t.projectDetailBankName}:</span><span className="font-medium">{project.committee.bank.bankName}</span><Button variant="outline" size="sm" onClick={()=>copy(project.committee.bank.bankName,'bankName')}>{copied==='bankName' ? t.projectDetailCopied : t.projectDetailCopy}</Button></div>
                      <div className="flex items-center justify-between gap-2"><span className="text-muted-foreground">{t.projectDetailAccountName}:</span><span className="font-medium">{project.committee.bank.accountName}</span><Button variant="outline" size="sm" onClick={()=>copy(project.committee.bank.accountName,'accName')}>{copied==='accName' ? t.projectDetailCopied : t.projectDetailCopy}</Button></div>
                      <div className="flex items-center justify-between gap-2"><span className="text-muted-foreground">{t.projectDetailAccountNumber}:</span><span className="font-mono text-sm font-medium">{project.committee.bank.accountNumber}</span><Button variant="outline" size="sm" onClick={()=>copy(project.committee.bank.accountNumber,'accNum')}>{copied==='accNum' ? t.projectDetailCopied : t.projectDetailCopy}</Button></div>
                    </div>
                    {project.committee.esewaId ? <div className="flex items-center justify-between gap-2 font-sans text-sm"><span className="text-muted-foreground">{t.projectDetailEsewa}:</span><span className="font-mono font-medium">{project.committee.esewaId}</span><Button variant="outline" size="sm" onClick={()=>copy(project.committee.esewaId!,'esewa')}>{copied==='esewa' ? t.projectDetailCopied : t.projectDetailCopy}</Button></div> : null}
                    {project.committee.khaltiId ? <div className="flex items-center justify-between gap-2 font-sans text-sm"><span className="text-muted-foreground">{t.projectDetailKhalti}:</span><span className="font-mono font-medium">{project.committee.khaltiId}</span><Button variant="outline" size="sm" onClick={()=>copy(project.committee.khaltiId!,'khalti')}>{copied==='khalti' ? t.projectDetailCopied : t.projectDetailCopy}</Button></div> : null}
                  </div>
                </>
              ) : <p className="font-sans text-xs text-muted-foreground">{t.projectsPaymentPending}</p>}
