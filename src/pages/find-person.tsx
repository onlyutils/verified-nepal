...
import { formatNumber, messageText } from "@/lib/format";
import { Byline, Headline, Rule, RuledTable, SectionLabel, SquareButton, Standfirst } from "@/components/legacy";
...
function DisclaimerBlock({ language, disclaimers }: { language: Language; disclaimers: string[] }) {
  ...
  return (
    <aside className="border-l border-ink pl-4 font-serif text-sm leading-6 text-muted-foreground">
      <p className="font-sans text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{t.officialDisclaimer}</p>
