...
- **Self-service helper delivery**: the owner-approved hybrid path now lets an individual helper or any member of a helper group take a published need, view private contact details, release it, and deliver it; the ledger, audit trail, claim-code confirmation, Desk release action, and Story eligibility are covered by backend tests.

- **Take/deliver e2e** ([findings/wave4-take.md](findings/wave4-take.md)): every rule passed (take/release/deliver, 3-take limit, contact access control, group take, assign-only, beneficiary confirmation, moderator release, story eligibility); it caught two pre-prod defects — the private-contact read returning 500 and the public ledger exposing an internal id — fixed in `6e371ca` with a Desk Boards disaster selector and a pointer query for the take limit, and re-verified on dev before the production deploy.

- **Delivery-chain e2e** ([findings/wave5-chain.md](findings/wave5-chain.md)): login policy, registrant dashboard, and the group → drop center → org hand-over → confirmation chain passed end to end (API and real UI); four polish items (hide "Mark delivered" while goods are en route, persistent drop-off instructions, a plural, the org tab-strip cue) shipped in batch 15 before the production deploy.

- **Guide page** `/how-to` (footer link): a presentation-ready walkthrough of everything the site does, 53 fresh screenshots, print stylesheet, full-screen image lightbox, and a dev-only "Try it on dev" credentials section (compiled out of production). Capturing it surfaced two more P1s — the Desk Posters tab crashing on a malformed date and a permanently empty public Projects board — both fixed (`3efdae9`) before the production deploy.
