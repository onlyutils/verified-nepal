// Cloudflare Pages Function for /get-help: static share card, no per-request data.
...
export const onRequestGet = async ({ request, next }: Ctx): Promise<Response> => {
  ...
  return applyMeta(page, withOrigin(STATIC_PAGE_META.getHelp, url.origin), `${canonicalOrigin(url.hostname)}/get-help`);
};
