// Cloudflare Pages Function for /give-help: static share card, no per-request data.
...
export const onRequestGet = async ({ request, next }: Ctx): Promise<Response> => {
  ...
  return applyMeta(page, withOrigin(STATIC_PAGE_META.giveHelp, url.origin), `${canonicalOrigin(url.hostname)}/give-help`);
};
